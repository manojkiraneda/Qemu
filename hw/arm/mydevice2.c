...
static void thread1_main(void) {
    ...
    printf("Thread 1 started. Execution period = %d uSecs\n",\
                                           exec_period_usecs);
    while(1) {
        status = mq_send(my_mq2, (const char*)&counter2, sizeof(counter2), 1);
        ASSERT(status != -1);
...
static void my_device_init(Object *obj)
{
    ...
    memory_region_init_io(&s->iomem, obj, &my_device_ops, s,"mydevice2",4);
